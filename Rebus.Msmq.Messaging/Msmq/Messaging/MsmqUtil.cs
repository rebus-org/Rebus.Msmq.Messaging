...
in order to specify a private queue named 'someQueue' on the machine with IP 10.0.1.45");
    ...
    public static void EnsureQueueExists(string inputQueuePath, ILog log = null, Action<MessageQueue> permissionsCallback = null)
    {
        ...
        try
        {
            ...
            SetPermissionForCurrentUser(newQueue);
            SetPermissionForLocalAdmins(newQueue);
    ...
    static void SetPermissionForLocalAdmins(MessageQueue newQueue)
    {
    ...
    static void SetPermissionForCurrentUser(MessageQueue newQueue)
    {
        newQueue.SetPermissions(WindowsIdentity.GetCurrent().Name, MessageQueueAccessRights.GenericWrite);
    }
